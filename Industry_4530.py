...
avg_intangibles_df.to_excel('avg_intangibles_to_mv_per_year_4530.xlsx', index=False)
df_N = df[df['sec_id'] == 7895].copy()
Nvidia = df_N.groupby(['sec_id', 'year'])[['market_cap', 'INT']].mean().reset_index()
Nvidia.to_excel('Nvidia.xlsx', index=False)
